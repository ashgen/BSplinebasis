 ...
 */
    ...
    throw
    BSplineException(ErrorCode::INCONSISTENT_DATA);
}
...
throw
BSplineException(ErrorCode::UNDETERMINED,
"Unsupported order of the derivative.");
}
if (bo.node == Node::FIRST) {
T power_of_dx1 = static_cast<T>(1);
for (
size_t i = bo.derivative;
i <=
order;
i++) {
s.
...
bspline::internal::facultyRatio<T>(i, i
- bo.derivative) *
power_of_dx1;
power_of_dx1 *=
dx1;
...
return
bspline::Spline<T, order>(std::move(x), std::move(coeffs)
);
}
